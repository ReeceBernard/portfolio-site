...
Replace the current text-based `Hero` component on the portfolio page with a full-bleed, animated 3D "double helix" of 20 photos — two strands of 10 images each, corkscrewing in CSS 3D space. The technique is ported from [githyperplexed/double-helix-gallery](https://github.com/githyperplexed/double-helix-gallery), adapted from a standalone vanilla TS project into a self-contained React component that drops into the existing portfolio.

## Goals

- A striking, personal hero section built from 20 photos of things the site owner likes.
- Faithful to the reference repo's motion and depth technique (CSS `perspective`, `rotateY` + `translateZ` + `translateY`, ambient rotation).
...
- React's role is limited to mounting the component, wiring up the loop/listeners in `useEffect`, and tearing them down on unmount (clearing the rAF, removing the resize listener).
...
- `src/pages/portfolio/components/helix/` — supporting modules ported from the reference (math helpers, viewport tracker, config), scoped to this component.
...
- **Reduced motion:** `prefers-reduced-motion` disables ambient rotation (elapsed time frozen at 0), matching the reference.

## Testing / verification

- Run the dev server and check the hero directly in a browser at both a desktop and a mobile viewport width.
- Verify `prefers-reduced-motion: reduce` stops ambient rotation.
